(function (global, factory) {
  ...
  function ChildReconciler(shouldTrackEffects) {
      ...
      function reconcilerSingleTextNode(returnFiber, currentFirstChild, textContent) {
          if (currentFirstChild) {
              //textNode直接复用
              const child = useFiber(currentFirstChild, textContent);
  ...
  const updateState = () => {
      ...
      return [hook.memoizedState, update.dispatch];
  };
  //mount hook执行逻辑
  ...
  const removeChild = (container, instance) => {
      container.removeChild(instance);
  };
  const updateText = (content, textNode) => {
  ...
  //处理DeletionFlag
  const handleDeletionFlag = (finishedFiber) => {
      {
  ...
  const CommitDeletion = (childFiber) => {
      let rootHostNode = null;
      commitNestedComponent(childFiber, (unmountFiber) => {
      ...
      if (rootHostNode != null) {
          ...
          if (parent != null) {
              //删除节点
              removeChild(parent, rootHostNode.stateNode);
          }
      }
      childFiber.return = null;
      childFiber.child = null;
  };
  const commitNestedComponent = (root, commitChildFn) => {
      let node = root;
      while (true) {
